...
st.markdown(
    ...
    Not only can they cope with greatly differing problem domains, they can also easily incorporate hard and soft constraints and find very good if
    not always optimal solutions.  
      
    And that is exactly the point I wanted to test. There is (in most cases) only one correct solution for a Sudoku Problem. If we measure 
    ...
    So the question was: 
    > **Can we build a genetic algorithm in such a way, that it avoids getting stuck in "good" local
    optima and instead reaches the global optimum?**
    """
    )
...
st.markdown(
    ...
    "simple" Business Intelligence applications I am constantly on the lookout for tools that allow me to easily build
    a professional looking and easy to use frontend, supporting the quick implementation of impactful POC and MVP.  
    ...
    **And then along came [streamlit](https://streamlit.io).** This newish library not only allows you to build the complete frontend in 
    ...
    good things about streamlit recently, I hadn't tried it out for myself before.  
      
    So this Genetic Sudoku Solver problem seemed like the perfect opportunity to learn and to try out the solutions this 
...
st.markdown(
    ...
    As for the frontend part, I'd like to thank the streamlit team for providing not only a perfect library but also a very concise and comprehensive documentation.
